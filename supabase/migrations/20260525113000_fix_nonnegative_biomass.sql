-- Biomass is a physical stock value and cannot be negative. Keep the recorded
-- event movements unchanged, but clamp analytical live inventory/biomass at zero
-- when historical movements overdraw a system.
...
facts as (
  select
    ...
    r.farm_id,
    r.system_name,
    case when greatest(r.number_of_fish, 0::double precision) > 0 then lineage.cycle_id::bigint else null::bigint end as production_cycle_id,
    r.growth_stage,
    r.system_is_active,
...
comment on view analytics.daily_system_facts is
  'Canonical daily model input layer. Uses operation lineage across transfers, ABW anchors for biomass, system volume for density, and clamps analytical live inventory/biomass at zero.';
