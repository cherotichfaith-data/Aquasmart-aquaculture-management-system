...
CREATE OR REPLACE FUNCTION "analytics"."debug_view_end"() RETURNS "text"
    LANGUAGE "plpgsql"
    AS $$
DECLARE v text;
BEGIN
...
CREATE OR REPLACE FUNCTION "analytics"."debug_view_update"() RETURNS "text"
    LANGUAGE "plpgsql"
    AS $$
DECLARE
  v_def text;
...
CREATE OR REPLACE FUNCTION "analytics"."update_production_summary_view"() RETURNS "void"
    LANGUAGE "plpgsql"
    AS $$
begin
...
CREATE OR REPLACE FUNCTION "private"."app_rpc_scope_ok"("p_farm_id" "uuid", "p_system_ids" bigint[] DEFAULT NULL::bigint[], "p_cycle_id" bigint DEFAULT NULL::bigint, "p_start_date" "date" DEFAULT NULL::"date", "p_end_date" "date" DEFAULT NULL::"date") RETURNS boolean
...
CREATE OR REPLACE FUNCTION "private"."assert_rpc_parameters"("p_farm_id" "uuid", "p_system_ids" bigint[] DEFAULT NULL::bigint[], "p_cycle_id" bigint DEFAULT NULL::bigint, "p_start_date" "date" DEFAULT NULL::"date", "p_end_date" "date" DEFAULT NULL::"date") RETURNS "void"
    LANGUAGE "plpgsql" STABLE SECURITY DEFINER
    SET "search_path" TO 'pg_catalog', 'public', 'private'
    AS $$
            BEGIN
...
CREATE OR REPLACE FUNCTION "private"."set_fish_mortality_farm_id"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'pg_catalog', 'public', 'private'
    AS $$
              BEGIN
...
CREATE OR REPLACE FUNCTION "public"."after_event_queue_new"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION "public"."after_event_queue_old"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'pg_catalog', 'public'
    AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION "public"."api_dashboard_batches"("p_farm_id" "uuid", "p_batch_ids" bigint[] DEFAULT NULL::bigint[], "p_stage" "public"."system_growth_stage" DEFAULT NULL::"public"."system_growth_stage", "p_start_date" "date" DEFAULT NULL::"date", "p_end_date" "date" DEFAULT NULL::"date") RETURNS TABLE("batch_id" bigint, "batch_name" "text", "growth_stage" "public"."system_growth_stage", "system_ids" bigint[], "system_count" integer, "input_start_date" "date", "input_end_date" "date", "as_of_date" "date", "fish_end" double precision, "biomass_end" double precision, "sampling_end_date" "date", "sample_age_days" integer, "efcr" double precision, "efcr_latest_date" "date", "efcr_arrow" "text", "feed_total" double precision, "abw" double precision, "abw_latest_date" "date", "abw_arrow" "text", "feeding_rate" double precision, "feeding_rate_latest_date" "date", "feeding_rate_arrow" "text", "mortality_rate" double precision, "mortality_rate_latest_date" "date", "mortality_rate_arrow" "text", "biomass_density" double precision, "biomass_density_latest_date" "date", "biomass_density_arrow" "text", "sgr" double precision, "agr" double precision, "sgr_arrow" "text", "agr_arrow" "text", "missing_days_count" integer, "water_quality_rating_average" "text", "water_quality_rating_numeric_average" double precision, "water_quality_latest_date" "date", "water_quality_arrow" "text", "worst_parameter" "text", "worst_parameter_value" double precision, "worst_parameter_unit" "text", "cycle_day" integer, "target_weight_g" double precision, "target_weight_progress_pct" double precision, "is_complete" boolean)
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO 'pg_catalog', 'public', 'analytics', 'private'
    AS $$ with batches_all as ( select fb.id as batch_id, coalesce(nullif(fb.name, ''), 'Batch #' || fb.id::text) as batch_name from public.fingerling_batch fb where fb.farm_id = p_farm_id and private.app_rpc_scope_ok(p_farm_id, null::bigint[], null::bigint, p_start_date, p_end_date) and (p_batch_ids is null or fb.id = any(p_batch_ids)) ), data_anchor as ( select coalesce(max(dsf.inventory_date), current_date) as last_data_date from analytics.daily_system_facts dsf join batches_all b on b.batch_id = dsf.batch_id ), bounds as ( select coalesce(p_start_date, da.last_data_date - interval '30 days')::date as start_date, coalesce(p_end_date, da.last_data_date)::date as end_date from data_anchor da ), period_meta as ( select b.start_date, b.end_date, greatest((b.end_date - b.start_date + 1)::integer, 1) as period_days from bounds b ), periods as ( select 'current'::text as period_label, pm.start_date, pm.end_date from period_meta pm union all select 'previous'::text, (pm.start_date - pm.period_days)::date, (pm.start_date - 1)::date from period_meta pm ), batch_flows as ( select fs.batch_id, fs.system_id, fs.number_of_fish_stocking::double precision as qty_delta from public.fish_stocking fs join batches_all b on b.batch_id = fs.batch_id where fs.system_id is not null and fs.date <= (select end_date from bounds) union all select ft.batch_id, ft.target_system_id, ft.number_of_fish_transfer::double precision from public.fish_transfer ft join batches_all b on b.batch_id = ft.batch_id where ft.target_system_id is not null and ft.date <= (select end_date from bounds) union all select ft.batch_id, ft.origin_system_id, -ft.number_of_fish_transfer::double precision from public.fish_transfer ft join batches_all b on b.batch_id = ft.batch_id where ft.origin_system_id is not null and ft.date <= (select end_date from bounds) union all select fm.batch_id, fm.system_id, -fm.number_of_fish_mortality::double precision from public.fish_mortality fm join batches_all b on b.batch_id = fm.batch_id where fm.system_id is not null and fm.date <= (select end_date from bounds) union all select fh.batch_id, fh.system_id, -coalesce(fh.number_of_fish_harvest, 0)::double precision from public.fish_harvest fh join batches_all b on b.batch_id = fh.batch_id where fh.system_id is not null and fh.date <= (select end_date from bounds) ), batch_system_balance as ( select batch_id, system_id, sum(qty_delta) as fish_balance from batch_flows group by batch_id, system_id having sum(qty_delta) > 0 ), batch_systems as ( select bsb.batch_id, bsb.system_id, s.growth_stage from batch_system_balance bsb join public.system s on s.id = bsb.system_id where s.farm_id = p_farm_id and s.is_active = true and coalesce(s.cage_status, 'occupied'::public.cage_status_enum) <> 'retired'::public.cage_status_enum ), batch_meta as ( select bs.batch_id, array_agg(distinct bs.system_id order by bs.system_id) as system_ids, count(distinct bs.system_id)::integer as system_count, case when count(distinct bs.growth_stage) = 1 then min(bs.growth_stage) else null end as growth_stage from batch_systems bs group by bs.batch_id ), batch as ( select ba.batch_id, ba.batch_name, bm.system_ids, bm.system_count, bm.growth_stage from batches_all ba join batch_meta bm on bm.batch_id = ba.batch_id where p_stage is null or exists (select 1 from batch_systems bs2 where bs2.batch_id = ba.batch_id and bs2.growth_stage = p_stage) ), inv_daily as ( select p.period_label, dsf.inventory_date, dsf.batch_id, sum(dsf.number_of_fish) as fish_count, sum(dsf.biomass_last_sampling) as biomass, case when sum(dsf.biomass_last_sampling) > 0 then sum(coalesce(dsf.abw_last_sampling,0) * coalesce(dsf.biomass_last_sampling,0)) / sum(dsf.biomass_last_sampling) else avg(dsf.abw_last_sampling) end as abw, max(dsf.last_abw_date) as sampling_end_date, case when sum(dsf.biomass_last_sampling) > 0 then sum(coalesce(dsf.feeding_rate,0) * coalesce(dsf.biomass_last_sampling,0)) / sum(dsf.biomass_last_sampling) else avg(dsf.feeding_rate) end as feeding_rate, case when sum(dsf.number_of_fish) > 0 then sum(coalesce(dsf.mortality_rate,0) * coalesce(dsf.number_of_fish,0)) / sum(dsf.number_of_fish) else avg(dsf.mortality_rate) end as mortality_rate, avg(dsf.biomass_density) as biomass_density from periods p join analytics.daily_system_facts dsf on dsf.inventory_date between p.start_date and p.end_date join batches_all b on b.batch_id = dsf.batch_id group by p.period_label, dsf.inventory_date, dsf.batch_id ), inv_snapshot as ( select distinct on (period_label, batch_id) period_label, batch_id, inventory_date, fish_count as fish_end, biomass as biomass_end, abw, sampling_end_date, feeding_rate, mortality_rate, biomass_density from inv_daily order by period_label, batch_id, inventory_date desc ), inv_latest as (select * from inv_snapshot where period_label = 'current'), inv_prev as (select * from inv_snapshot where period_label = 'previous'), inv_agg as ( select batch_id, count(distinct inventory_date)::integer as days_present from inv_daily where period_label = 'current' group by batch_id ), inv_period_metrics as ( select period_label, batch_id, case when sum(coalesce(fish_count,0)) > 0 then sum(coalesce(mortality_rate,0) * coalesce(fish_count,0)) / sum(fish_count) else avg(mortality_rate) end as mortality_rate_period, avg(biomass_density) as biomass_density_period, case when sum(coalesce(biomass,0)) > 0 then sum(coalesce(feeding_rate,0) * coalesce(biomass,0)) / sum(biomass) else avg(feeding_rate) end as feeding_rate_period from inv_daily group by period_label, batch_id ), inv_current_metrics as (select * from inv_period_metrics where period_label = 'current'), inv_prev_metrics as (select * from inv_period_metrics where period_label = 'previous'), ps_window as ( select p.period_label, pc.batch_id, ps.date, ps.feed_over_period, ps.biomass_increase_over_period, ps.sgr, ps.agr, ps.days_in_period from periods p join analytics.production_summary ps on ps.date between p.start_date and p.end_date join public.production_cycle pc on pc.cycle_id = ps.cycle_id join batches_all b on b.batch_id = pc.batch_id ), ps_daily as ( select period_label, batch_id, date, sum(coalesce(feed_over_period,0))::double precision as feed_over_period, greatest(sum(coalesce(biomass_increase_over_period,0)),0)::double precision as biomass_increase_over_period, case when sum(case when sgr is not null then greatest(coalesce(biomass_increase_over_period,0),0) else 0 end) > 0 then sum(coalesce(sgr,0) * greatest(coalesce(biomass_increase_over_period,0),0)) / sum(case when sgr is not null then greatest(coalesce(biomass_increase_over_period,0),0) else 0 end) else null end::double precision as sgr, case when sum(case when agr is not null then greatest(coalesce(biomass_increase_over_period,0),0) else 0 end) > 0 then sum(coalesce(agr,0) * greatest(coalesce(biomass_increase_over_period,0),0)) / sum(case when agr is not null then greatest(coalesce(biomass_increase_over_period,0),0) else 0 end) else null end::double precision as agr, max(days_in_period)::integer as days_in_period from ps_window group by period_label, batch_id, date ), ps_ranked as ( select *, row_number() over (partition by period_label, batch_id order by date desc) as rn from ps_daily ), ps_latest as ( select * from ps_ranked where period_label = 'current' and rn = 1 ), ps_period_metrics as ( select period_label, batch_id, sum(coalesce(feed_over_period,0))::double precision as feed_total, sum(coalesce(feed_over_period,0))::double precision as total_feed_period, sum(coalesce(biomass_increase_over_period,0))::double precision as total_growth_period, case when sum(case when sgr is not null then days_in_period else 0 end) > 0 then sum(case when sgr is not null then sgr * days_in_period else 0 end) / nullif(sum(case when sgr is not null then days_in_period else 0 end), 0) else null end::double precision as sgr_period, case when sum(case when agr is not null then days_in_period else 0 end) > 0 then sum(case when agr is not null then agr * days_in_period else 0 end) / nullif(sum(case when agr is not null then days_in_period else 0 end), 0) else null end::double precision as agr_period from ps_daily group by period_label, batch_id ), ps_current_metrics as ( select period_label, batch_id, feed_total, case when total_growth_period > 0 then (total_feed_period/total_growth_period)::double precision else null::double precision end as efcr_period, sgr_period, agr_period from ps_period_metrics where period_label = 'current' ), ps_prev_metrics as ( select period_label, batch_id, feed_total, case when total_growth_period > 0 then (total_feed_period/total_growth_period)::double precision else null::double precision end as efcr_period, sgr_period, agr_period from ps_period_metrics where period_label = 'previous' ), wq_window as ( select p.period_label, bs.batch_id, wq.rating_date, wq.rating_numeric, wq.worst_parameter, wq.worst_parameter_value, wq.worst_parameter_unit, wq.created_at, wq.id from periods p join public.daily_water_quality_rating wq on wq.rating_date between p.start_date and p.end_date join batch_systems bs on bs.system_id = wq.system_id ), wq_avg as ( select period_label, batch_id, avg(rating_numeric::double precision) as rating_numeric_avg, case when avg(rating_numeric::double precision) >= 2.5 then 'Optimal' when avg(rating_numeric::double precision) >= 1.5 then 'Acceptable' when avg(rating_numeric::double precision) >= 0.5 then 'Critical' else 'Lethal' end as rating_label_avg from wq_window group by period_label, batch_id ), wq_current_avg as (select * from wq_avg where period_label = 'current'), wq_prev_avg as (select * from wq_avg where period_label = 'previous'), wq_ranked as ( select wq.period_label, wq.batch_id, wq.rating_date, wq.worst_parameter, wq.worst_parameter_value, wq.worst_parameter_unit, row_number() over ( partition by wq.period_label, wq.batch_id order by wq.rating_date desc, wq.rating_numeric asc, wq.created_at desc, wq.id desc ) as rn from wq_window wq ), wq_latest as (select * from wq_ranked where period_label = 'current' and rn = 1), batch_cycles as ( select pc.batch_id, pc.cycle_start, pc.target_weight_g from public.production_cycle pc join batches_all ba on ba.batch_id = pc.batch_id where pc.ongoing_cycle = true ) select batch.batch_id, batch.batch_name, batch.growth_stage, batch.system_ids, batch.system_count, b.start_date as input_start_date, b.end_date as input_end_date, b.end_date as as_of_date, inv_latest.fish_end, inv_latest.biomass_end, inv_latest.sampling_end_date, case when inv_latest.sampling_end_date is null then null else (b.end_date - inv_latest.sampling_end_date)::integer end as sample_age_days, case when ps_latest.biomass_increase_over_period > 0 then (ps_latest.feed_over_period / ps_latest.biomass_increase_over_period)::double precision else null::double precision end as efcr, ps_latest.date as efcr_latest_date, case when ps_current_metrics.efcr_period is null or ps_prev_metrics.efcr_period is null then null when ps_current_metrics.efcr_period = ps_prev_metrics.efcr_period then 'straight' when ps_current_metrics.efcr_period > ps_prev_metrics.efcr_period then 'up' else 'down' end as efcr_arrow, ps_current_metrics.feed_total, inv_latest.abw, inv_latest.sampling_end_date as abw_latest_date, case when inv_latest.abw is null or inv_prev.abw is null then null when inv_latest.abw = inv_prev.abw then 'straight' when inv_latest.abw > inv_prev.abw then 'up' else 'down' end as abw_arrow, inv_current_metrics.feeding_rate_period as feeding_rate, inv_latest.inventory_date as feeding_rate_latest_date, case when inv_current_metrics.feeding_rate_period is null or inv_prev_metrics.feeding_rate_period is null then null when inv_current_metrics.feeding_rate_period = inv_prev_metrics.feeding_rate_period then 'straight' when inv_current_metrics.feeding_rate_period > inv_prev_metrics.feeding_rate_period then 'up' else 'down' end as feeding_rate_arrow, inv_latest.mortality_rate, inv_latest.inventory_date as mortality_rate_latest_date, case when inv_current_metrics.mortality_rate_period is null or inv_prev_metrics.mortality_rate_period is null then null when inv_current_metrics.mortality_rate_period = inv_prev_metrics.mortality_rate_period then 'straight' when inv_current_metrics.mortality_rate_period > inv_prev_metrics.mortality_rate_period then 'up' else 'down' end as mortality_rate_arrow, inv_latest.biomass_density, inv_latest.inventory_date as biomass_density_latest_date, case when inv_current_metrics.biomass_density_period is null or inv_prev_metrics.biomass_density_period is null then null when inv_current_metrics.biomass_density_period = inv_prev_metrics.biomass_density_period then 'straight' when inv_current_metrics.biomass_density_period > inv_prev_metrics.biomass_density_period then 'up' else 'down' end as biomass_density_arrow, ps_current_metrics.sgr_period as sgr, ps_current_metrics.agr_period as agr, case when ps_current_metrics.sgr_period is null or ps_prev_metrics.sgr_period is null then null when ps_current_metrics.sgr_period > ps_prev_metrics.sgr_period then 'up' when ps_current_metrics.sgr_period < ps_prev_metrics.sgr_period then 'down' else 'straight' end as sgr_arrow, case when ps_current_metrics.agr_period is null or ps_prev_metrics.agr_period is null then null when ps_current_metrics.agr_period > ps_prev_metrics.agr_period then 'up' when ps_current_metrics.agr_period < ps_prev_metrics.agr_period then 'down' else 'straight' end as agr_arrow, greatest(0, (b.end_date - b.start_date + 1)::integer - coalesce(inv_agg.days_present, 0)) as missing_days_count, wq_current_avg.rating_label_avg as water_quality_rating_average, wq_current_avg.rating_numeric_avg as water_quality_rating_numeric_average, wq_latest.rating_date as water_quality_latest_date, case when wq_current_avg.rating_numeric_avg is null or wq_prev_avg.rating_numeric_avg is null then null when wq_current_avg.rating_numeric_avg = wq_prev_avg.rating_numeric_avg then 'straight' when wq_current_avg.rating_numeric_avg > wq_prev_avg.rating_numeric_avg then 'up' else 'down' end as water_quality_arrow, wq_latest.worst_parameter, wq_latest.worst_parameter_value, wq_latest.worst_parameter_unit, case when bc.cycle_start is null then null else (b.end_date - bc.cycle_start)::integer end as cycle_day, bc.target_weight_g::double precision as target_weight_g, case when bc.target_weight_g is not null and inv_latest.abw is not null then round((inv_latest.abw / bc.target_weight_g::double precision * 100)::numeric, 1)::double precision else null end as target_weight_progress_pct, case when inv_latest.fish_end is not null and inv_latest.fish_end > 0 and inv_latest.biomass_end is not null and ps_current_metrics.feed_total is not null and ps_latest.biomass_increase_over_period is not null and ps_latest.biomass_increase_over_period > 0 and inv_latest.abw is not null and inv_latest.biomass_density is not null then true else false end as is_complete from batch cross join bounds b left join inv_latest on inv_latest.batch_id = batch.batch_id left join inv_prev on inv_prev.batch_id = batch.batch_id left join inv_agg on inv_agg.batch_id = batch.batch_id left join ps_latest on ps_latest.batch_id = batch.batch_id left join inv_current_metrics on inv_current_metrics.batch_id = batch.batch_id left join inv_prev_metrics on inv_prev_metrics.batch_id = batch.batch_id left join ps_current_metrics on ps_current_metrics.batch_id = batch.batch_id left join ps_prev_metrics on ps_prev_metrics.batch_id = batch.batch_id left join wq_current_avg on wq_current_avg.batch_id = batch.batch_id left join wq_prev_avg on wq_prev_avg.batch_id = batch.batch_id left join wq_latest on wq_latest.batch_id = batch.batch_id left join batch_cycles bc on bc.batch_id = batch.batch_id order by batch.batch_name; $$;


ALTER FUNCTION "public"."api_dashboard_batches"("p_farm_id" "uuid", "p_batch_ids" bigint[], "p_stage" "public"."system_growth_stage", "p_start_date" "date", "p_end_date" "date") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."api_feed_efcr_trend"("p_farm_id" "uuid", "p_system_ids" bigint[] DEFAULT NULL::bigint[], "p_start_date" "date" DEFAULT NULL::"date", "p_end_date" "date" DEFAULT NULL::"date") RETURNS TABLE("date" "date", "efcr_period" numeric)
    LANGUAGE "plpgsql" STABLE SECURITY DEFINER
    SET "search_path" TO 'pg_catalog', 'public', 'analytics', 'private'
    AS $$
declare
...
CREATE OR REPLACE FUNCTION "public"."api_feed_inventory_feed_type_options_rpc"("p_farm_id" "uuid", "p_date_to" "date" DEFAULT NULL::"date") RETURNS TABLE("id" bigint, "farm_id" "uuid", "feed_line" "text", "label" "text", "feed_category" "text", "feed_pellet_size" "text", "crude_protein_percentage" numeric, "crude_fat_percentage" numeric, "visibility_scope" "text")
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO 'pg_catalog', 'public'
    AS $$
...
CREATE OR REPLACE FUNCTION "public"."api_feed_plan_vs_actual"("p_farm_id" "uuid", "p_system_ids" bigint[] DEFAULT NULL::bigint[], "p_start_date" "date" DEFAULT NULL::"date", "p_end_date" "date" DEFAULT NULL::"date") RETURNS TABLE("date" "date", "planned_feed_kg" numeric, "actual_feed_kg" numeric)
    LANGUAGE "plpgsql" STABLE SECURITY DEFINER
    SET "search_path" TO 'pg_catalog', 'public', 'analytics', 'private'
    AS $$
declare
...
CREATE OR REPLACE FUNCTION "public"."api_feed_vs_biomass_gain"("p_farm_id" "uuid", "p_system_ids" bigint[] DEFAULT NULL::bigint[], "p_start_date" "date" DEFAULT NULL::"date", "p_end_date" "date" DEFAULT NULL::"date") RETURNS TABLE("system_id" bigint, "system_name" "text", "date" "date", "feed_kg" numeric, "biomass_gain_kg" numeric)
    LANGUAGE "plpgsql" STABLE SECURITY DEFINER
    SET "search_path" TO 'pg_catalog', 'public', 'analytics', 'private'
    AS $$
declare
...
CREATE OR REPLACE FUNCTION "public"."api_feeding_alerts"("p_farm_id" "uuid", "p_system_ids" bigint[] DEFAULT NULL::bigint[], "p_start_date" "date" DEFAULT NULL::"date", "p_end_date" "date" DEFAULT NULL::"date") RETURNS TABLE("system_id" bigint, "system_name" "text", "date" "date", "alert" "text", "recommendation" "text", "severity" "text")
    LANGUAGE "plpgsql" STABLE SECURITY DEFINER
    SET "search_path" TO 'pg_catalog', 'public', 'analytics', 'private'
    AS $$
declare
...
CREATE OR REPLACE FUNCTION "public"."api_feeding_rate_vs_target"("p_farm_id" "uuid", "p_system_ids" bigint[] DEFAULT NULL::bigint[], "p_start_date" "date" DEFAULT NULL::"date", "p_end_date" "date" DEFAULT NULL::"date") RETURNS TABLE("date" "date", "actual_rate" numeric, "feed_rate_min_pct" numeric, "feed_rate_max_pct" numeric)
    LANGUAGE "plpgsql" STABLE SECURITY DEFINER
    SET "search_path" TO 'pg_catalog', 'public', 'analytics', 'private'
    AS $$
declare
...
CREATE OR REPLACE FUNCTION "public"."api_feeding_response_distribution"("p_farm_id" "uuid", "p_system_ids" bigint[] DEFAULT NULL::bigint[], "p_start_date" "date" DEFAULT NULL::"date", "p_end_date" "date" DEFAULT NULL::"date") RETURNS TABLE("feeding_response" integer, "count" bigint)
    LANGUAGE "plpgsql" STABLE SECURITY DEFINER
    SET "search_path" TO 'pg_catalog', 'public', 'analytics', 'private'
    AS $$
declare
...
CREATE OR REPLACE FUNCTION "public"."api_latest_water_quality_status"("p_farm_id" "uuid", "p_system_id" bigint DEFAULT NULL::bigint) RETURNS TABLE("system_id" bigint, "system_name" "text", "rating_date" "date", "rating" "text", "rating_numeric" integer, "worst_parameter" "text", "worst_parameter_value" double precision, "worst_parameter_unit" "text")
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO 'pg_catalog', 'public'
    AS $$
...
CREATE OR REPLACE FUNCTION "public"."classify_growth_phase"("p_abw_g" numeric, "p_scenario" "text" DEFAULT 'main'::"text") RETURNS TABLE("phase_id" integer, "scenario" "text", "abw_min_g" numeric, "abw_max_g" numeric, "sgr_pct_per_day" numeric)
    LANGUAGE "sql" STABLE
    SET "search_path" TO 'pg_catalog', 'public'
    AS $$
  select
...
CREATE OR REPLACE FUNCTION "public"."current_fish_count"("p_system_id" bigint) RETURNS numeric
    LANGUAGE "sql" STABLE
    AS $$
  SELECT
      COALESCE((SELECT SUM(number_of_fish_stocking) FROM fish_stocking WHERE system_id = p_system_id), 0)
    + COALESCE((SELECT SUM(number_of_fish_transfer) FROM fish_transfer WHERE target_system_id = p_system_id), 0)
    - COALESCE((SELECT SUM(number_of_fish_transfer) FROM fish_transfer WHERE origin_system_id = p_system_id), 0)
    - COALESCE((SELECT SUM(number_of_fish_mortality) FROM fish_mortality WHERE system_id = p_system_id), 0);
$$;


ALTER FUNCTION "public"."current_fish_count"("p_system_id" bigint) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."enforce_api_rate_limit"("p_scope" "text", "p_user_id" "uuid", "p_limit" integer, "p_window_seconds" integer, "p_ip_address" "inet" DEFAULT NULL::"inet") RETURNS TABLE("allowed" boolean, "current_count" integer, "remaining" integer, "reset_at" timestamp with time zone)
    LANGUAGE "plpgsql" SECURITY DEFINER
...
ALTER FUNCTION "public"."enforce_api_rate_limit"("p_scope" "text", "p_user_id" "uuid", "p_limit" integer, "p_window_seconds" integer, "p_ip_address" "inet") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."feed_inventory_snapshot_kg"("p_bag_weight" numeric, "p_amount_of_bags" numeric, "p_opened_bags" numeric) RETURNS numeric
    LANGUAGE "sql" IMMUTABLE
    SET "search_path" TO 'pg_catalog', 'public'
    AS $$
  select greatest(
...
CREATE OR REPLACE FUNCTION "public"."refresh_feeding_model_after_config_change"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'pg_catalog', 'public', 'analytics'
    AS $$
begin
...
CREATE OR REPLACE FUNCTION "public"."refresh_feeding_model_output"() RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'pg_catalog', 'public', 'analytics'
    AS $$
begin
...
CREATE OR REPLACE FUNCTION "public"."resolve_abw_g"("p_total_weight_kg" double precision, "p_fish_count" double precision) RETURNS double precision
    LANGUAGE "sql" IMMUTABLE
    SET "search_path" TO 'pg_catalog', 'public'
    AS $$
  SELECT CASE
...
CREATE OR REPLACE FUNCTION "public"."sync_cage_status"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'pg_catalog', 'public'
    AS $$ DECLARE v_system_id bigint; v_count numeric; BEGIN IF TG_TABLE_NAME = 'fish_transfer' THEN IF COALESCE(NEW.origin_system_id, OLD.origin_system_id) IS NOT NULL THEN v_system_id := COALESCE(NEW.origin_system_id, OLD.origin_system_id); v_count := public.current_fish_count(v_system_id); UPDATE public.system SET cage_status = CASE WHEN v_count <= 0 THEN 'available'::cage_status_enum ELSE 'occupied'::cage_status_enum END WHERE id = v_system_id AND cage_status <> 'retired'::cage_status_enum; END IF; IF COALESCE(NEW.target_system_id, OLD.target_system_id) IS NOT NULL THEN v_system_id := COALESCE(NEW.target_system_id, OLD.target_system_id); v_count := public.current_fish_count(v_system_id); UPDATE public.system SET cage_status = CASE WHEN v_count > 0 THEN 'occupied'::cage_status_enum ELSE 'available'::cage_status_enum END WHERE id = v_system_id AND cage_status <> 'retired'::cage_status_enum; END IF; ELSE v_system_id := COALESCE(NEW.system_id, OLD.system_id); IF v_system_id IS NOT NULL THEN v_count := public.current_fish_count(v_system_id); UPDATE public.system SET cage_status = CASE WHEN v_count > 0 THEN 'occupied'::cage_status_enum ELSE 'available'::cage_status_enum END WHERE id = v_system_id AND cage_status <> 'retired'::cage_status_enum; END IF; END IF; RETURN NULL; END; $$;


ALTER FUNCTION "public"."sync_cage_status"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."sync_water_quality_measured_at_parts"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'pg_catalog', 'public'
    AS $$
BEGIN
...
CREATE OR REPLACE VIEW "analytics"."efcr_trend" AS
 SELECT "s"."farm_id",
    "ps"."system_id",
    "s"."name" AS "system_name",
    "ps"."date",
...
ALTER TABLE "analytics"."efcr_trend" OWNER TO "postgres";
...
ALTER TABLE "analytics"."feeding_model_output" OWNER TO "postgres";


CREATE OR REPLACE VIEW "analytics"."feed_dashboard_kpis" AS
 WITH "base" AS (
         SELECT "s"."farm_id",
            "fmo"."system_id",
            "fmo"."date",
...
ALTER TABLE "analytics"."feed_dashboard_kpis" OWNER TO "postgres";


CREATE OR REPLACE VIEW "analytics"."feed_plan_vs_actual" AS
 SELECT "s"."farm_id",
    "fmo"."system_id",
    "s"."name" AS "system_name",
    "fmo"."date",
...
ALTER TABLE "analytics"."feed_plan_vs_actual" OWNER TO "postgres";


CREATE OR REPLACE VIEW "analytics"."feed_vs_biomass_gain" AS
 SELECT "s"."farm_id",
    "dsf"."system_id",
    "s"."name" AS "system_name",
    "dsf"."inventory_date" AS "date",
...
ALTER TABLE "analytics"."feed_vs_biomass_gain" OWNER TO "postgres";


CREATE OR REPLACE VIEW "analytics"."feeding_alerts" AS
 SELECT "s"."farm_id",
    "fmo"."system_id",
    "s"."name" AS "system_name",
    "fmo"."date",
...
ALTER TABLE "analytics"."feeding_alerts" OWNER TO "postgres";


CREATE OR REPLACE VIEW "analytics"."feeding_model_validation" AS
...
ALTER TABLE "analytics"."feeding_model_validation" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "public"."feeding_rate_config" (
...
CREATE OR REPLACE VIEW "analytics"."feeding_rate_vs_target" AS
 SELECT "s"."farm_id",
    "fmo"."system_id",
    "s"."name" AS "system_name",
    "fmo"."date",
...
ALTER TABLE "analytics"."feeding_rate_vs_target" OWNER TO "postgres";


CREATE OR REPLACE VIEW "analytics"."feeding_response_distribution" AS
 SELECT "s"."farm_id",
    "fr"."system_id",
    "fr"."date",
    "fr"."feeding_response",
...
ALTER TABLE "analytics"."feeding_response_distribution" OWNER TO "postgres";


CREATE OR REPLACE VIEW "analytics"."system_feed_status" AS
 SELECT "s"."farm_id",
    "s"."id" AS "system_id",
    "s"."name" AS "system_name",
    "fmo"."date",
...
ALTER TABLE "analytics"."system_feed_status" OWNER TO "postgres";
...
ALTER TABLE "public"."api_rate_limit_counter" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."growth_phase" (
    "phase_id" integer NOT NULL,
    "scenario" "text" DEFAULT 'main'::"text" NOT NULL,
    "abw_min_g" numeric(10,3) NOT NULL,
    "abw_max_g" numeric(10,3),
...
COMMENT ON TABLE "public"."production_cycle" IS 'One biological production cycle per fish batch. system_id records the starting placement only; movements and splits are recorded by stocking/transfer/operation system fields.';
...
CREATE TABLE IF NOT EXISTS "public"."production_cycle_system" (
    "id" bigint NOT NULL,
    "cycle_id" bigint NOT NULL,
    "system_id" bigint NOT NULL,
    "previous_system_id" bigint,
...
ALTER TABLE "public"."production_cycle_system" OWNER TO "postgres";


ALTER TABLE "public"."production_cycle_system" ALTER COLUMN "id" ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME "public"."production_cycle_system_id_seq"
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE IF NOT EXISTS "public"."system_name_change_log" (
    "id" bigint NOT NULL,
    "system_id" bigint NOT NULL,
...
ALTER TABLE ONLY "public"."api_rate_limit_counter"
    ADD CONSTRAINT "api_rate_limit_counter_pkey" PRIMARY KEY ("scope", "user_id", "window_start");
...
ALTER TABLE ONLY "public"."production_cycle_system"
    ADD CONSTRAINT "production_cycle_system_pkey" PRIMARY KEY ("id");
...
CREATE INDEX "api_rate_limit_counter_lookup_idx" ON "public"."api_rate_limit_counter" USING "btree" ("scope", "user_id", "window_start" DESC);
...
CREATE OR REPLACE TRIGGER "after_feeding_record_del_inventory" AFTER DELETE ON "public"."feeding_record" REFERENCING OLD TABLE AS "old_rows" FOR EACH STATEMENT EXECUTE FUNCTION "public"."after_event_queue_old"();



CREATE OR REPLACE TRIGGER "after_feeding_record_ins_inventory" AFTER INSERT ON "public"."feeding_record" REFERENCING NEW TABLE AS "new_rows" FOR EACH STATEMENT EXECUTE FUNCTION "public"."after_event_queue_new"();



CREATE OR REPLACE TRIGGER "after_feeding_record_upd_inventory" AFTER UPDATE ON "public"."feeding_record" REFERENCING NEW TABLE AS "new_rows" FOR EACH STATEMENT EXECUTE FUNCTION "public"."after_event_queue_new"();



CREATE OR REPLACE TRIGGER "after_fish_harvest_del_inventory" AFTER DELETE ON "public"."fish_harvest" REFERENCING OLD TABLE AS "old_rows" FOR EACH STATEMENT EXECUTE FUNCTION "public"."after_event_queue_old"();



CREATE OR REPLACE TRIGGER "after_fish_harvest_ins_inventory" AFTER INSERT ON "public"."fish_harvest" REFERENCING NEW TABLE AS "new_rows" FOR EACH STATEMENT EXECUTE FUNCTION "public"."after_event_queue_new"();



CREATE OR REPLACE TRIGGER "after_fish_harvest_upd_inventory" AFTER UPDATE ON "public"."fish_harvest" REFERENCING NEW TABLE AS "new_rows" FOR EACH STATEMENT EXECUTE FUNCTION "public"."after_event_queue_new"();



CREATE OR REPLACE TRIGGER "after_fish_mortality_del_inventory" AFTER DELETE ON "public"."fish_mortality" REFERENCING OLD TABLE AS "old_rows" FOR EACH STATEMENT EXECUTE FUNCTION "public"."after_event_queue_old"();



CREATE OR REPLACE TRIGGER "after_fish_mortality_ins_inventory" AFTER INSERT ON "public"."fish_mortality" REFERENCING NEW TABLE AS "new_rows" FOR EACH STATEMENT EXECUTE FUNCTION "public"."after_event_queue_new"();



CREATE OR REPLACE TRIGGER "after_fish_mortality_upd_inventory" AFTER UPDATE ON "public"."fish_mortality" REFERENCING NEW TABLE AS "new_rows" FOR EACH STATEMENT EXECUTE FUNCTION "public"."after_event_queue_new"();



CREATE OR REPLACE TRIGGER "after_fish_sampling_del_inventory" AFTER DELETE ON "public"."fish_sampling_weight" REFERENCING OLD TABLE AS "old_rows" FOR EACH STATEMENT EXECUTE FUNCTION "public"."after_event_queue_old"();



CREATE OR REPLACE TRIGGER "after_fish_sampling_ins_inventory" AFTER INSERT ON "public"."fish_sampling_weight" REFERENCING NEW TABLE AS "new_rows" FOR EACH STATEMENT EXECUTE FUNCTION "public"."after_event_queue_new"();



CREATE OR REPLACE TRIGGER "after_fish_sampling_upd_inventory" AFTER UPDATE ON "public"."fish_sampling_weight" REFERENCING NEW TABLE AS "new_rows" FOR EACH STATEMENT EXECUTE FUNCTION "public"."after_event_queue_new"();



CREATE OR REPLACE TRIGGER "after_fish_stocking_del_inventory" AFTER DELETE ON "public"."fish_stocking" REFERENCING OLD TABLE AS "old_rows" FOR EACH STATEMENT EXECUTE FUNCTION "public"."after_event_queue_old"();



CREATE OR REPLACE TRIGGER "after_fish_stocking_ins_inventory" AFTER INSERT ON "public"."fish_stocking" REFERENCING NEW TABLE AS "new_rows" FOR EACH STATEMENT EXECUTE FUNCTION "public"."after_event_queue_new"();



CREATE OR REPLACE TRIGGER "after_fish_stocking_upd_inventory" AFTER UPDATE ON "public"."fish_stocking" REFERENCING NEW TABLE AS "new_rows" FOR EACH STATEMENT EXECUTE FUNCTION "public"."after_event_queue_new"();



CREATE OR REPLACE TRIGGER "after_fish_transfer_del_inventory" AFTER DELETE ON "public"."fish_transfer" REFERENCING OLD TABLE AS "old_rows" FOR EACH STATEMENT EXECUTE FUNCTION "public"."after_event_queue_old"();



CREATE OR REPLACE TRIGGER "after_fish_transfer_ins_inventory" AFTER INSERT ON "public"."fish_transfer" REFERENCING NEW TABLE AS "new_rows" FOR EACH STATEMENT EXECUTE FUNCTION "public"."after_event_queue_new"();



CREATE OR REPLACE TRIGGER "after_fish_transfer_upd_inventory" AFTER UPDATE ON "public"."fish_transfer" REFERENCING NEW TABLE AS "new_rows" FOR EACH STATEMENT EXECUTE FUNCTION "public"."after_event_queue_new"();
...
CREATE OR REPLACE TRIGGER "trg_farm_updated_at" BEFORE UPDATE ON "public"."farm" FOR EACH ROW EXECUTE FUNCTION "private"."set_updated_at"();



CREATE OR REPLACE TRIGGER "trg_feed_inventory_updated_at" BEFORE UPDATE ON "public"."feed_inventory" FOR EACH ROW EXECUTE FUNCTION "private"."set_updated_at"();



CREATE OR REPLACE TRIGGER "trg_feed_type_updated_at" BEFORE UPDATE ON "public"."feed_type" FOR EACH ROW EXECUTE FUNCTION "private"."set_updated_at"();



CREATE OR REPLACE TRIGGER "trg_feeding_rate_config_refresh_feeding_model" AFTER INSERT OR DELETE OR UPDATE OR TRUNCATE ON "public"."feeding_rate_config" FOR EACH STATEMENT EXECUTE FUNCTION "public"."refresh_feeding_model_after_config_change"();
...
CREATE OR REPLACE TRIGGER "trg_feeding_record_updated_at" BEFORE UPDATE ON "public"."feeding_record" FOR EACH ROW EXECUTE FUNCTION "private"."set_updated_at"();



CREATE OR REPLACE TRIGGER "trg_fingerling_batch_updated_at" BEFORE UPDATE ON "public"."fingerling_batch" FOR EACH ROW EXECUTE FUNCTION "private"."set_updated_at"();
...
CREATE OR REPLACE TRIGGER "trg_fish_harvest_updated_at" BEFORE UPDATE ON "public"."fish_harvest" FOR EACH ROW EXECUTE FUNCTION "private"."set_updated_at"();
...
CREATE OR REPLACE TRIGGER "trg_fish_mortality_set_farm_id" BEFORE INSERT OR UPDATE ON "public"."fish_mortality" FOR EACH ROW EXECUTE FUNCTION "private"."set_fish_mortality_farm_id"();



CREATE OR REPLACE TRIGGER "trg_fish_mortality_updated_at" BEFORE UPDATE ON "public"."fish_mortality" FOR EACH ROW EXECUTE FUNCTION "private"."set_updated_at"();
...
CREATE OR REPLACE TRIGGER "trg_fish_sampling_weight_updated_at" BEFORE UPDATE ON "public"."fish_sampling_weight" FOR EACH ROW EXECUTE FUNCTION "private"."set_updated_at"();
...
CREATE OR REPLACE TRIGGER "trg_fish_stocking_updated_at" BEFORE UPDATE ON "public"."fish_stocking" FOR EACH ROW EXECUTE FUNCTION "private"."set_updated_at"();
...
CREATE OR REPLACE TRIGGER "trg_fish_transfer_updated_at" BEFORE UPDATE ON "public"."fish_transfer" FOR EACH ROW EXECUTE FUNCTION "private"."set_updated_at"();



CREATE OR REPLACE TRIGGER "trg_growth_phase_refresh_feeding_model" AFTER INSERT OR DELETE OR UPDATE OR TRUNCATE ON "public"."growth_phase" FOR EACH STATEMENT EXECUTE FUNCTION "public"."refresh_feeding_model_after_config_change"();
...
CREATE OR REPLACE TRIGGER "trg_organization_updated_at" BEFORE UPDATE ON "public"."organization" FOR EACH ROW EXECUTE FUNCTION "private"."set_updated_at"();
...
CREATE OR REPLACE TRIGGER "trg_production_cycle_updated_at" BEFORE UPDATE ON "public"."production_cycle" FOR EACH ROW EXECUTE FUNCTION "private"."set_updated_at"();



CREATE OR REPLACE TRIGGER "trg_sync_cage_status_mortality" AFTER INSERT OR DELETE OR UPDATE ON "public"."fish_mortality" FOR EACH ROW EXECUTE FUNCTION "public"."sync_cage_status"();



CREATE OR REPLACE TRIGGER "trg_sync_cage_status_stocking" AFTER INSERT OR DELETE OR UPDATE ON "public"."fish_stocking" FOR EACH ROW EXECUTE FUNCTION "public"."sync_cage_status"();



CREATE OR REPLACE TRIGGER "trg_sync_cage_status_transfer" AFTER INSERT OR DELETE OR UPDATE ON "public"."fish_transfer" FOR EACH ROW EXECUTE FUNCTION "public"."sync_cage_status"();



CREATE OR REPLACE TRIGGER "trg_system_updated_at" BEFORE UPDATE ON "public"."system" FOR EACH ROW EXECUTE FUNCTION "private"."set_updated_at"();



CREATE OR REPLACE TRIGGER "trg_user_settings_updated_at" BEFORE UPDATE ON "public"."user_settings" FOR EACH ROW EXECUTE FUNCTION "private"."set_updated_at"();



CREATE OR REPLACE TRIGGER "trg_water_quality_measurement_updated_at" BEFORE UPDATE ON "public"."water_quality_measurement" FOR EACH ROW EXECUTE FUNCTION "private"."set_updated_at"();



CREATE OR REPLACE TRIGGER "trg_water_quality_sync_measured_at_parts" BEFORE INSERT OR UPDATE OF "measured_at", "date", "time" ON "public"."water_quality_measurement" FOR EACH ROW EXECUTE FUNCTION "public"."sync_water_quality_measured_at_parts"();
...
ALTER TABLE ONLY "public"."feeding_record"
    ADD CONSTRAINT "feeding_record_cycle_batch_fkey" FOREIGN KEY ("cycle_id", "batch_id") REFERENCES "public"."production_cycle"("cycle_id", "batch_id");
...
ALTER TABLE ONLY "public"."fish_harvest"
    ADD CONSTRAINT "fish_harvest_cycle_batch_fkey" FOREIGN KEY ("cycle_id", "batch_id") REFERENCES "public"."production_cycle"("cycle_id", "batch_id");
...
ALTER TABLE ONLY "public"."fish_mortality"
    ADD CONSTRAINT "fish_mortality_cycle_batch_fkey" FOREIGN KEY ("cycle_id", "batch_id") REFERENCES "public"."production_cycle"("cycle_id", "batch_id");
...
ALTER TABLE ONLY "public"."fish_sampling_weight"
    ADD CONSTRAINT "fish_sampling_weight_cycle_batch_fkey" FOREIGN KEY ("cycle_id", "batch_id") REFERENCES "public"."production_cycle"("cycle_id", "batch_id");
...
ALTER TABLE ONLY "public"."fish_stocking"
    ADD CONSTRAINT "fish_stocking_cycle_batch_fkey" FOREIGN KEY ("cycle_id", "batch_id") REFERENCES "public"."production_cycle"("cycle_id", "batch_id");
...
ALTER TABLE ONLY "public"."fish_transfer"
    ADD CONSTRAINT "fish_transfer_cycle_batch_fkey" FOREIGN KEY ("cycle_id", "batch_id") REFERENCES "public"."production_cycle"("cycle_id", "batch_id");
...
ALTER TABLE ONLY "public"."production_cycle_system"
    ADD CONSTRAINT "pcs_cycle_id_fkey" FOREIGN KEY ("cycle_id") REFERENCES "public"."production_cycle"("cycle_id");



ALTER TABLE ONLY "public"."production_cycle_system"
    ADD CONSTRAINT "pcs_previous_system_id_fkey" FOREIGN KEY ("previous_system_id") REFERENCES "public"."system"("id");



ALTER TABLE ONLY "public"."production_cycle_system"
    ADD CONSTRAINT "pcs_system_id_fkey" FOREIGN KEY ("system_id") REFERENCES "public"."system"("id");
...
CREATE POLICY "feeding_record: update by managers" ON "public"."feeding_record" FOR UPDATE USING ((EXISTS ( SELECT 1
   FROM ("public"."system" "s"
     JOIN "public"."farm_user" "fu" ON (("fu"."farm_id" = "s"."farm_id")))
  WHERE (("s"."id" = "feeding_record"."system_id") AND ("fu"."user_id" = "auth"."uid"()) AND ("fu"."role" = ANY (ARRAY['admin'::"text", 'farm_manager'::"text", 'system_operator'::"text"])))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM ("public"."system" "s"
     JOIN "public"."farm_user" "fu" ON (("fu"."farm_id" = "s"."farm_id")))
...
CREATE POLICY "production_cycle: delete by managers" ON "public"."production_cycle" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM ("public"."fingerling_batch" "fb"
...
ALTER TABLE "public"."production_cycle_system" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "production_cycle_update" ON "public"."production_cycle" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM ("public"."fingerling_batch" "fb"
     JOIN "public"."farm_user" "fu" ON (("fu"."farm_id" = "fb"."farm_id")))
  WHERE (("fb"."id" = "production_cycle"."batch_id") AND ("fu"."user_id" = ( SELECT "auth"."uid"() AS "uid")))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM (("public"."fingerling_batch" "fb"
     JOIN "public"."farm_user" "fu" ON (("fu"."farm_id" = "fb"."farm_id")))
     JOIN "public"."system" "s" ON ((("s"."id" = "production_cycle"."system_id") AND ("s"."farm_id" = "fb"."farm_id"))))
  WHERE (("fb"."id" = "production_cycle"."batch_id") AND ("fu"."user_id" = ( SELECT "auth"."uid"() AS "uid"))))));
...
GRANT ALL ON FUNCTION "public"."api_dashboard_consolidated"("p_farm_id" "uuid", "p_system_ids" bigint[], "p_stage" "public"."system_growth_stage", "p_start_date" "date", "p_end_date" "date", "p_time_period" "text", "p_limit" integer, "p_order_desc" boolean) TO "authenticated";
GRANT ALL ON FUNCTION "public"."api_dashboard_consolidated"("p_farm_id" "uuid", "p_system_ids" bigint[], "p_stage" "public"."system_growth_stage", "p_start_date" "date", "p_end_date" "date", "p_time_period" "text", "p_limit" integer, "p_order_desc" boolean) TO "service_role";



GRANT ALL ON FUNCTION "public"."api_dashboard_systems"("p_farm_id" "uuid", "p_system_ids" bigint[], "p_stage" "public"."system_growth_stage", "p_start_date" "date", "p_end_date" "date") TO "authenticated";
GRANT ALL ON FUNCTION "public"."api_dashboard_systems"("p_farm_id" "uuid", "p_system_ids" bigint[], "p_stage" "public"."system_growth_stage", "p_start_date" "date", "p_end_date" "date") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."api_time_period_bounds_scoped"("p_farm_id" "uuid", "p_time_period" "text", "p_scope" "text", "p_anchor_date" "date", "p_system_id" bigint, "p_batch_id" bigint) TO "anon";
...
REVOKE ALL ON FUNCTION "public"."enforce_api_rate_limit"("p_scope" "text", "p_user_id" "uuid", "p_limit" integer, "p_window_seconds" integer, "p_ip_address" "inet") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."enforce_api_rate_limit"("p_scope" "text", "p_user_id" "uuid", "p_limit" integer, "p_window_seconds" integer, "p_ip_address" "inet") TO "service_role";
...
GRANT SELECT ON TABLE "analytics"."daily_system_facts" TO "anon";
...
GRANT SELECT ON TABLE "analytics"."production_summary" TO "anon";
...
GRANT SELECT ON TABLE "analytics"."efcr_trend" TO "anon";
GRANT SELECT ON TABLE "analytics"."efcr_trend" TO "authenticated";



GRANT SELECT ON TABLE "analytics"."feeding_model_output" TO "anon";
GRANT SELECT ON TABLE "analytics"."feeding_model_output" TO "authenticated";



GRANT SELECT ON TABLE "analytics"."feed_dashboard_kpis" TO "anon";
GRANT SELECT ON TABLE "analytics"."feed_dashboard_kpis" TO "authenticated";



GRANT SELECT ON TABLE "analytics"."feed_plan_vs_actual" TO "anon";
GRANT SELECT ON TABLE "analytics"."feed_plan_vs_actual" TO "authenticated";



GRANT SELECT ON TABLE "analytics"."feed_vs_biomass_gain" TO "anon";
GRANT SELECT ON TABLE "analytics"."feed_vs_biomass_gain" TO "authenticated";



GRANT SELECT ON TABLE "analytics"."feeding_alerts" TO "anon";
GRANT SELECT ON TABLE "analytics"."feeding_alerts" TO "authenticated";



GRANT SELECT ON TABLE "analytics"."feeding_model_validation" TO "anon";
GRANT SELECT ON TABLE "analytics"."feeding_model_validation" TO "authenticated";
...
GRANT SELECT ON TABLE "analytics"."feeding_rate_vs_target" TO "anon";
GRANT SELECT ON TABLE "analytics"."feeding_rate_vs_target" TO "authenticated";



GRANT SELECT ON TABLE "analytics"."feeding_response_distribution" TO "anon";
GRANT SELECT ON TABLE "analytics"."feeding_response_distribution" TO "authenticated";



GRANT SELECT ON TABLE "analytics"."system_feed_status" TO "anon";
GRANT SELECT ON TABLE "analytics"."system_feed_status" TO "authenticated";
...
GRANT REFERENCES,TRIGGER,TRUNCATE ON TABLE "public"."api_rate_limit_counter" TO "anon";
GRANT REFERENCES,TRIGGER,TRUNCATE ON TABLE "public"."api_rate_limit_counter" TO "authenticated";
GRANT REFERENCES,TRIGGER,TRUNCATE ON TABLE "public"."api_rate_limit_counter" TO "service_role";
...
GRANT REFERENCES,TRIGGER,TRUNCATE ON TABLE "public"."production_cycle_system" TO "anon";
GRANT REFERENCES,TRIGGER,TRUNCATE ON TABLE "public"."production_cycle_system" TO "authenticated";
GRANT REFERENCES,TRIGGER,TRUNCATE ON TABLE "public"."production_cycle_system" TO "service_role";



GRANT UPDATE ON SEQUENCE "public"."production_cycle_system_id_seq" TO "anon";
GRANT UPDATE ON SEQUENCE "public"."production_cycle_system_id_seq" TO "authenticated";
GRANT UPDATE ON SEQUENCE "public"."production_cycle_system_id_seq" TO "service_role";
