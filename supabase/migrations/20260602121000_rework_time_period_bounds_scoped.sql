drop function if exists public.api_time_period_bounds_scoped(uuid, text, text, date);
...
comment on function public.api_time_period_bounds_scoped(uuid, text, text, date, bigint) is
  'Intentional app-facing SECURITY DEFINER RPC. Direct access to internal read models is revoked; callers must be authenticated and function body must enforce farm membership/scope checks.';
