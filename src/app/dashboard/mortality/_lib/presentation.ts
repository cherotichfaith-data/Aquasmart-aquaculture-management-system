...
export function formatNumber(value: number | null | undefined, digits = 1) {
  return formatNumberValue(value, { decimals: digits, minimumDecimals: digits, fallback: "N/A" })
}
...
export function riskBadgeClass(row: MortalityRiskRow) {
  ...
  if (row.atRiskScore >= 4) return "bg-warning/10 text-warning border-warning/30"
...
export function investigationBadgeClass(status: InvestigationStatus) {
  ...
  if (status === "monitoring") return "bg-warning/10 text-warning border-warning/30"
  if (status === "resolved") return "bg-success/10 text-success border-success/30"
