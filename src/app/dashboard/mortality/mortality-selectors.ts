...
type MortalityReadingParameter = "dissolved_oxygen" | "temperature"
...
const MORTALITY_READING_PARAMETERS = new Set<MortalityReadingParameter>(["dissolved_oxygen", "temperature"])
...
export function buildLatestReadingsBySystem(rows: WaterQualityMeasurementRow[]) {
  const readingsBySystem = buildLatestParameterizedReadingsBySystem<MortalityReadingParameter>(
    rows,
    MORTALITY_READING_PARAMETERS,
  )
  const map = new Map<number, LatestReading>()

  readingsBySystem.forEach((entry, systemId) => {
    map.set(systemId, {
      doValue: entry.readings.dissolved_oxygen ?? null,
      doTimestamp: entry.timestamps.dissolved_oxygen ?? null,
      temperatureValue: entry.readings.temperature ?? null,
      temperatureTimestamp: entry.timestamps.temperature ?? null,
    })
  })
