...
export function BatchQuickCreate({ farmId, systemId = null, onCreated }: BatchQuickCreateProps) {
  ...
  }, [createdSuppliers, suppliersQuery.data])
  ...
  const supplierOptionsToken = useMemo(
    () => Symbol(suppliers.map((supplier) => supplier.id).join(",")),
    [suppliers],
  )
  const [supplierIdDraft, setSupplierIdDraft] = useState(() => ({
    sourceToken: supplierOptionsToken,
    value: "",
  }))
  const supplierId =
    supplierIdDraft.sourceToken === supplierOptionsToken && suppliers.some((supplier) => String(supplier.id) === supplierIdDraft.value)
      ? supplierIdDraft.value
      : suppliers.length > 0
        ? String(suppliers[0]?.id ?? "")
        : ""
  const setSupplierId = (value: SetStateAction<string>) => {
    setSupplierIdDraft((current) => {
      const previousValue =
        current.sourceToken === supplierOptionsToken && suppliers.some((supplier) => String(supplier.id) === current.value)
          ? current.value
          : suppliers.length > 0
            ? String(suppliers[0]?.id ?? "")
            : ""
