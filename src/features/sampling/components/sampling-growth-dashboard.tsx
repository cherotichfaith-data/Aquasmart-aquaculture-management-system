  ...
  const curveOptions = useMemo<ChartOptions<"line">>(
    () =>
      buildCartesianOptions({
        ...
        tooltip: {
          callbacks: {
            title: (items: TooltipItem<"line">[]) => `Age ${curveAll.ageDomain[items[0]?.dataIndex ?? 0] ?? "-"} days`,
            label: (context: TooltipItem<"line">) => `${context.dataset.label}: ${Number(context.parsed.y).toFixed(1)} g`,
          },
        },
      }),
  ...
  const curveUnder100Options = useMemo<ChartOptions<"line">>(
    () =>
      buildCartesianOptions({
        ...
        tooltip: {
          callbacks: {
            title: (items: TooltipItem<"line">[]) => `Age ${curveUnder100.ageDomain[items[0]?.dataIndex ?? 0] ?? "-"} days`,
            label: (context: TooltipItem<"line">) => `${context.dataset.label}: ${Number(context.parsed.y).toFixed(1)} g`,
          },
        },
      }),
