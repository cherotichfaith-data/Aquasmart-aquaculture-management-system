...
export default function AbwGrowthByBatchChart({
  ...
  const data = useMemo<ChartData<"line">>(() => {
    ...
    for (const point of points) {
      ...
      byBatch.get(point.batch_id)!.set(point.sample_date, point.abw_g)
    }
    ...
    return (
      <Card className="rounded-2xl">
        ...
        <CardContent className="pt-2">
          <EmptyState title="No growth samples yet" description="Growth data will appear once batches have samples." />
        </CardContent>
      </Card>
    )
  }
